## Modern Next.js E-Commerce Store

A fully configurable e-commerce site built with Next.js, featuring a clean and modern design with complete shopping cart functionality.

Out of the box, the site uses static data from a provider at `providers/inventoryProvider.js`. You can easily update this provider to fetch data from any real API by modifying the call in the `getInventory` function.
...
To see an example of how to process payments server-side with stripe, check out the Lambda function example for handling Stripe payments.
